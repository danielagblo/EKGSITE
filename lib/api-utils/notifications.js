...
async function sendAdminOrderEmail(order) {
  ...
  if (resendKey) {
    try {
      ...
      if (!error) return { ok: true, via: 'resend', data }
      console.error('Admin Resend SDK Error', error)
    } catch (e) {
      console.error('Admin Resend catch Error', e)
    }
  }
...
async function sendCustomerOrderEmail(order) {
  ...
  if (resendKey) {
    try {
      ...
      if (!error) return { ok: true, via: 'resend', data }
      console.error('Customer Resend SDK Error', error)
    } catch (e) {
      console.error('Customer Resend catch Error', e)
    }
  }
